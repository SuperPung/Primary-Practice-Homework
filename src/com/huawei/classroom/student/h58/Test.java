...
public class Test {
	...
	public static void main(String[] args) {
		...
		for (Map.Entry<String, Integer> entry : entrySet) {
			double off = 0.0;
			if ("wang".equals(entry.getKey())) {
				off = Math.abs(entry.getValue() - 3358) / 3358.0;
			} else if ("zhao".equals(entry.getKey())) {
				off = Math.abs(entry.getValue() - 6664) / 6664.0;
			} else if ("zhang".equals(entry.getKey())) {
				off = Math.abs(entry.getValue() - 3306) / 3306.0;
			} else if ("li".equals(entry.getKey())) {
				off = Math.abs(entry.getValue() - 3332) / 3332.0;
			}
			off = Math.round(off * 10000) / 100.0;
			System.out.println(entry.getKey() + " 的误差为：" + off + "%");
		}
